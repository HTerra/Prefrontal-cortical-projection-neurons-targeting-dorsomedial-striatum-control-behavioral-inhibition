function [psthBinsValue, PSTH, TrialDistr] = PSTH_event_SD_CorIncOm(All, neuronIndex, parameters_OI, neuron, binSize, event, InfoDist, varargin)
...
%                         end
                ...
                shadedErrorBar(time_axis_TrialStart,smooth(PSTH.Inc.trial_start,3),SEM,{'r'});
                SEM = nanstd(psthBinsValue.Om.trial_start,1)/sqrt(size(psthBinsValue.Om.trial_start,1)); 
                shadedErrorBar(time_axis_TrialStart,smooth(PSTH.Om.trial_start),SEM,{'b'});
...
%                 shadedErrorBar(time_axis_TrialStart,PSTH.Prem.trial_start,SEM,{'c'});
                ...
                line([-Treshold_tBef Treshold_tAft],[mean(PSTH.Cor.trial_start(1:10)) mean(PSTH.Cor.trial_start(1:10))])

                SEM = nanstd(psthBinsValue.Inc.wait_start,1)/sqrt(size(psthBinsValue.Inc.wait_start,1)); 
                shadedErrorBar(time_axis_Treshold,smooth(PSTH.Inc.wait_start,3),SEM,{'r'});
                SEM = nanstd(psthBinsValue.Om.wait_start,1)/sqrt(size(psthBinsValue.Om.wait_start,1)); 
                shadedErrorBar(time_axis_Treshold,smooth(PSTH.Om.wait_start,3),SEM,{'b'});
...
%                 axis([-Treshold_tBef Treshold_tAft 0 inf])
                ...
                shadedErrorBar(time_axis_Response,smooth(PSTH.Inc.resp,3),SEM,{'r'});
                hold on
                SEM = nanstd(psthBinsValue.Om.resp,1)/sqrt(size(psthBinsValue.Om.resp,1)); 
                shadedErrorBar(time_axis_Response,smooth(PSTH.Om.resp,3),SEM,{'b'});
...
% 
                for i=1:numel(TrialDistr.IncTresholdIncluded)
                    ...
                    scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Inc.trial_start(TrialDistr.IncTresholdIncluded(i)),repmat(n,numel(spnum),1)',dotSize,'r','filled');
                    scatter(eventTimes.Inc.resp(TrialDistr.IncTresholdIncluded(i))-eventTimes.Inc.trial_start(TrialDistr.IncTresholdIncluded(i)),repmat(n,numel(1),1)',dotSize*2,'c','filled');
                    n = n+1;
                end
% 
                for i=1:numel(TrialDistr.OmTresholdIncluded)
                    ...
                    scatter(eventTimes.Om.resp(TrialDistr.OmTresholdIncluded(i))-eventTimes.Om.trial_start(TrialDistr.OmTresholdIncluded(i)),repmat(n,numel(1),1)',dotSize*2,'c','filled');
                    n = n+1;
                end
...
%                     n = n+1;
%                 end
                axis([-TrialStart_tBef TrialStart_tAft 1 numel(TrialDistr.CorTresholdIncluded)+numel(TrialDistr.IncTresholdIncluded)+numel(TrialDistr.OmTresholdIncluded)])
                ylabel('Trial')
...
% 
                for i=1:numel(TrialDistr.IncTresholdIncluded)
                    ...
                    scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Inc.wait_start(TrialDistr.IncTresholdIncluded(i)),repmat(n,numel(spnum),1)',dotSize,'r','filled');
                    n = n+1;
                end
                for i=1:numel(TrialDistr.OmTresholdIncluded)
...
%                     n = n+1;
%                 end
                axis([-Treshold_tBef Treshold_tAft 1 numel(TrialDistr.CorTresholdIncluded)+numel(TrialDistr.IncTresholdIncluded)+numel(TrialDistr.OmTresholdIncluded)])
                ylabel('Trial')
...
% 
                for i=1:numel(TrialDistr.IncTresholdIncluded)
                    ...
                    scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Inc.resp(TrialDistr.IncTresholdIncluded(i)),repmat(n,numel(spnum),1)',dotSize,'r','filled');
                    n = n+1;
                end
...
%                     n = n+1;
%                 end
                axis([-Response_tBef Response_tAft 1 numel(TrialDistr.CorTresholdIncluded)+numel(TrialDistr.IncTresholdIncluded)+numel(TrialDistr.OmTresholdIncluded)])
                ylabel('Trial')
