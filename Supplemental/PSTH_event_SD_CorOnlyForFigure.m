function [psthBinsValue, PSTH, TrialDistr,bins] = PSTH_event_SD_CorOnlyForFigure(All, neuronIndex, parameters_OI, neuron, binSize, event, InfoDist, varargin)
...
Cue_tAft = 4;
    ...
    [Correct, ~, ~, ~, ~, ~, ~]  = CC_ephys_var_ITI_SD_TTL_Master(10,1,All{5, 1}{neuron, 2});
...
%                 time_axis_Treshold = round(linspace(-Treshold_tBef,Treshold_tAft,((Treshold_tBef+Treshold_tAft-binSize)/binSize)),1);
%                 time_axis_TrialStart = round(linspace(-TrialStart_tBef,TrialStart_tAft,((TrialStart_tBef+TrialStart_tAft-binSize)/binSize)),1);
%                 time_axis_Response = round(linspace(-Response_tBef,Response_tAft,((Response_tBef+Response_tAft-binSize)/binSize)),1);
                time_axis_Treshold = -Treshold_tBef:binSize:Treshold_tAft-binSize;
                time_axis_TrialStart = -TrialStart_tBef:binSize:TrialStart_tAft-binSize;
                time_axis_Cue = -2:binSize:4-binSize;
                time_axis_Response = -Response_tBef:binSize:Response_tAft-binSize;
                
                CorSD2_include = find(ismember(TrialDistr.CorTresholdIncluded, Correct.TrialPosSD2));
                CorSD5_include = find(ismember(TrialDistr.CorTresholdIncluded, Correct.TrialPosSD5));
                CorSD10_include = find(ismember(TrialDistr.CorTresholdIncluded, Correct.TrialPosSD10));
                
                subplot(3,4,5)
                hold on
                SEM_10 = nanstd(psthBinsValue.Cor.trial_start(CorSD10_include,end-29:end),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI125_include,:),1)); 
                shadedErrorBar(time_axis_Cue,mean(psthBinsValue.Cor.trial_start(CorSD10_include,end-29:end),1)*(1/binSize),SEM_10,{'k'});
                SEM_5 = nanstd(psthBinsValue.Cor.trial_start(CorSD5_include,end-29:end),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI125_include,:),1)); 
                shadedErrorBar(time_axis_Cue,mean(psthBinsValue.Cor.trial_start(CorSD5_include,end-29:end),1)*(1/binSize),SEM_5,{'c'});
                SEM_2 = nanstd(psthBinsValue.Cor.trial_start(CorSD2_include,end-29:end),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI125_include,:),1)); 
                shadedErrorBar(time_axis_Cue,mean(psthBinsValue.Cor.trial_start(CorSD2_include,end-29:end),1)*(1/binSize),SEM_2,{'y'});
...
%                 axis([-TrialStart_tBef TrialStart_tAft 0 inf])
                subplot(3,4,6)
                hold on
                SEM_10 = nanstd(psthBinsValue.Cor.wait_start(CorSD10_include,:),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI125_include,:),1)); 
                shadedErrorBar(time_axis_Treshold,mean(psthBinsValue.Cor.wait_start(CorSD10_include,:),1)*(1/binSize),SEM_10,{'k'});
                SEM_5 = nanstd(psthBinsValue.Cor.wait_start(CorSD5_include,:),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI125_include,:),1)); 
                shadedErrorBar(time_axis_Treshold,mean(psthBinsValue.Cor.wait_start(CorSD5_include,:),1)*(1/binSize),SEM_5,{'c'});
                SEM_2 = nanstd(psthBinsValue.Cor.wait_start(CorSD2_include,:),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI125_include,:),1)); 
                shadedErrorBar(time_axis_Treshold,mean(psthBinsValue.Cor.wait_start(CorSD2_include,:),1)*(1/binSize),SEM_2,{'y'});
...
%                 axis([-Response_tBef Response_tAft 0 inf])
                ...
                hold on
                n = 1;
                for i=1:numel(CorSD10_include)
                    spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.trial_start(CorSD10_include(i))+3 & All{21, 1}{neuron, 1} < eventTimes.Cor.trial_start(CorSD10_include(i))+9);
                    scatter(All{21, 1}{neuron, 1}(spnum)-(eventTimes.Cor.trial_start(CorSD10_include(i))+5),repmat(n,numel(spnum),1)',0.3,'k','filled');
                    %scatter(-eventVideoBehavCor.treshold(CorSD10_include(i)),repmat(n,numel(1),1)',0.3,'r','filled');
                    %scatter(-eventVideoBehavCor.treshold(CorSD10_include(i))+(eventTimes.Cor.resp(CorSD10_include(i))-eventTimes.Cor.trial_start(CorSD10_include(i))),repmat(n,numel(1),1)',0.5,'b','filled');
                    n = n+1;
                end
                for i=1:numel(CorSD5_include)
                    spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.trial_start(CorSD5_include(i))+3 & All{21, 1}{neuron, 1} < eventTimes.Cor.trial_start(CorSD5_include(i))+9);
                    scatter(All{21, 1}{neuron, 1}(spnum)-(eventTimes.Cor.trial_start(CorSD5_include(i))+5),repmat(n,numel(spnum),1)',0.3,'c','filled');
                    %scatter(-eventVideoBehavCor.trial_start(CorSD5_include(i)),repmat(n,numel(1),1)',0.3,'r','filled');
                    %scatter(-eventVideoBehavCor.treshold(CorSD5_include(i))+(eventTimes.Cor.resp(CorSD5_include(i))-eventTimes.Cor.trial_start(CorSD5_include(i))),repmat(n,numel(1),1)',0.5,'b','filled');
                    n = n+1;
                end
                for i=1:numel(CorSD2_include)
                    spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.trial_start(CorSD2_include(i))+3 & All{21, 1}{neuron, 1} < eventTimes.Cor.trial_start(CorSD2_include(i))+9);
                    scatter(All{21, 1}{neuron, 1}(spnum)-(eventTimes.Cor.trial_start(CorSD2_include(i))+5),repmat(n,numel(spnum),1)',0.3,'y','filled');
                    %scatter(-eventVideoBehavCor.trial_start(CorSD2_include(i)),repmat(n,numel(1),1)',0.3,'r','filled');
                    %scatter(-eventVideoBehavCor.treshold(CorSD2_include(i))+(eventTimes.Cor.resp(CorSD2_include(i))-eventTimes.Cor.trial_start(CorSD2_include(i))),repmat(n,numel(1),1)',0.5,'b','filled');
                    n = n+1;
                end
...
%                     scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Om.trial_start(TrialDistr.OmTresholdIncluded(i)),repmat(n,numel(spnum),1)',0.5,'b','filled');
%                     n = n+1;
%                 end
...
%                     n = n+1;
%                 end
                axis([-Cue_tBef Cue_tAft 1 numel(TrialDistr.CorTresholdIncluded)])
                ylabel('Trial')
                ...
                n = 1;
                for i=1:numel(CorSD10_include)
                    spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.wait_start(CorSD10_include(i))-Treshold_tBef & All{21, 1}{neuron, 1} < eventTimes.Cor.wait_start(CorSD10_include(i))+Treshold_tAft);
                    scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Cor.wait_start(CorSD10_include(i)),repmat(n,numel(spnum),1)',0.3,'k','filled');
                    scatter(-eventVideoBehavCor.treshold(CorSD10_include(i)),repmat(n,numel(1),1)',0.3,'r','filled');
                    %scatter(-eventVideoBehavCor.treshold(CorSD10_include(i))+(eventTimes.Cor.resp(CorSD10_include(i))-eventTimes.Cor.trial_start(CorSD10_include(i))),repmat(n,numel(1),1)',0.5,'b','filled');
                    n = n+1;
                end
                for i=1:numel(CorSD5_include)
                    spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.wait_start(CorSD5_include(i))-Treshold_tBef & All{21, 1}{neuron, 1} < eventTimes.Cor.wait_start(CorSD5_include(i))+Treshold_tAft);
                    scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Cor.wait_start(CorSD5_include(i)),repmat(n,numel(spnum),1)',0.3,'c','filled');
                    scatter(-eventVideoBehavCor.treshold(CorSD5_include(i)),repmat(n,numel(1),1)',0.3,'r','filled');
                    %scatter(-eventVideoBehavCor.treshold(CorSD5_include(i))+(eventTimes.Cor.resp(CorSD5_include(i))-eventTimes.Cor.trial_start(CorSD5_include(i))),repmat(n,numel(1),1)',0.5,'b','filled');
                    n = n+1;
                end
                for i=1:numel(CorSD2_include)
                    spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.wait_start(CorSD2_include(i))-Treshold_tBef & All{21, 1}{neuron, 1} < eventTimes.Cor.wait_start(CorSD2_include(i))+Treshold_tAft);
                    scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Cor.wait_start(CorSD2_include(i)),repmat(n,numel(spnum),1)',0.3,'y','filled');
                    scatter(-eventVideoBehavCor.treshold(CorSD2_include(i)),repmat(n,numel(1),1)',0.3,'r','filled');
                    %scatter(-eventVideoBehavCor.treshold(CorSD2_include(i))+(eventTimes.Cor.resp(CorSD2_include(i))-eventTimes.Cor.trial_start(CorSD2_include(i))),repmat(n,numel(1),1)',0.5,'b','filled');
                    n = n+1;
                end
% 
...
%                     scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Om.wait_start(TrialDistr.OmTresholdIncluded(i)),repmat(n,numel(spnum),1)',0.5,'b','filled');
%                     n = n+1;
%                 end
%                 for i=1:numel(TrialDistr.PremTresholdIncluded)
...
%                     scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Om.resp(TrialDistr.OmTresholdIncluded(i)),repmat(n,numel(spnum),1)',0.5,'b','filled');
%                     n = n+1;
%                 end
%                 for i=1:numel(TrialDistr.PremTresholdIncluded)
