function [psthBinsValue, PSTH, TrialDistr, bins] = PSTH_event_ITI_CorOnlyForFigure(All, neuronIndex, parameters_OI, neuron, binSize, event, InfoDist,varargin)
...
%             time_axis_Cue = round(linspace(-Cue_tBef+(binSize),Cue_tAft,((Cue_tBef+Cue_tAft)/binSize)),1);
            ...
            time_axis_Cue = -Cue_tBef:binSize:Cue_tAft-binSize;
...
%             shadedErrorBar(time_axis_TrialStart,PSTH.Prem.trial_start,SEM,{'c'});
            ...
            %CorITI5_include = find(ismember(TrialDistr.CorTresholdIncluded, Correct.TrialPosITI50));
            %CorITI75_include = find(ismember(TrialDistr.CorTresholdIncluded, Correct.TrialPosITI75));
            %CorITI125_include = find(ismember(TrialDistr.CorTresholdIncluded, Correct.TrialPosITI125));

            subplot(3,4,6)
            hold on
            SEM_125 = nanstd(psthBinsValue.Cor.wait_start,1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI125_include,:),1)); 
            shadedErrorBar(time_axis_Treshold,mean(psthBinsValue.Cor.wait_start,1)*(1/binSize),SEM_125,{'k'});
%             SEM_75 = nanstd(psthBinsValue.Cor.wait_start(CorITI75_include,:),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI75_include,:),1)); 
%             shadedErrorBar(time_axis_Treshold,mean(psthBinsValue.Cor.wait_start(CorITI75_include,:),1)*(1/binSize),SEM_75,{'c'});
%             SEM_50 = nanstd(psthBinsValue.Cor.wait_start(CorITI5_include,:),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI5_include,:),1)); 
%             shadedErrorBar(time_axis_Treshold,mean(psthBinsValue.Cor.wait_start(CorITI5_include,:),1)*(1/binSize),SEM_50,{'y'});
            axis([-Treshold_tBef Treshold_tAft 0 inf])

            subplot(3,4,7)
            hold on
            SEM_125 = nanstd(psthBinsValue.Cor.cue,1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI125_include,:),1)); 
            shadedErrorBar(time_axis_Cue,mean(psthBinsValue.Cor.cue,1)*(1/binSize),SEM_125,{'k'});
%             SEM_75 = nanstd(psthBinsValue.Cor.cue(CorITI75_include,:),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI75_include,:),1)); 
%             shadedErrorBar(time_axis_Cue,mean(psthBinsValue.Cor.cue(CorITI75_include,:),1)*(1/binSize),SEM_75,{'c'});
%             SEM_50 = nanstd(psthBinsValue.Cor.cue(CorITI5_include,:),1);%/sqrt(size(psthBinsValue.Cor.wait_start(CorITI5_include,:),1)); 
%             shadedErrorBar(time_axis_Cue,mean(psthBinsValue.Cor.cue(CorITI5_include,:),1)*(1/binSize),SEM_50,{'y'});
...
%             end
            ...
            hold on
            n = 1;
%             CorITI5_include = Correct.TrialPosITI50(ismember(Correct.TrialPosITI50, TrialDistr.CorTresholdIncluded));
%             for i=1:numel(CorITI5_include)
%                 spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.wait_start(CorITI5_include(i))-Treshold_tBef & All{21, 1}{neuron, 1} < eventTimes.Cor.wait_start(CorITI5_include(i))+Treshold_tAft);
%                 scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Cor.wait_start(CorITI5_include(i)),repmat(n,numel(spnum),1)',0.3,'k','filled');
%                 scatter(-eventVideoBehavCor.treshold(CorITI5_include(i)),repmat(n,numel(1),1)',0.3,'r','filled');
%                 %scatter(-eventVideoBehavCor.treshold(CorITI5_include(i))+(eventTimes.Cor.resp(CorITI5_include(i))-eventTimes.Cor.trial_start(CorITI5_include(i))),repmat(n,numel(1),1)',0.5,'b','filled');
%                 n = n+1;
%             end
%             CorITI75_include = Correct.TrialPosITI75(ismember(Correct.TrialPosITI75, TrialDistr.CorTresholdIncluded));
%             for i=1:numel(CorITI75_include)
%                 spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.wait_start(CorITI75_include(i))-Treshold_tBef & All{21, 1}{neuron, 1} < eventTimes.Cor.wait_start(CorITI75_include(i))+Treshold_tAft);
%                 scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Cor.wait_start(CorITI75_include(i)),repmat(n,numel(spnum),1)',0.3,'c','filled');
%                 scatter(-eventVideoBehavCor.treshold(CorITI75_include(i)),repmat(n,numel(1),1)',0.3,'r','filled');
%                 %scatter(-eventVideoBehavCor.treshold(CorITI75_include(i))+(eventTimes.Cor.resp(CorITI75_include(i))-eventTimes.Cor.trial_start(CorITI75_include(i))),repmat(n,numel(1),1)',0.5,'b','filled');
%                 n = n+1;
%             end
            CorITI125_include = Correct.TrialPosITI125(ismember(Correct.TrialPosITI125, TrialDistr.CorTresholdIncluded));
            for i=1:numel(CorITI125_include)
                spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.wait_start(CorITI125_include(i))-Treshold_tBef & All{21, 1}{neuron, 1} < eventTimes.Cor.wait_start(CorITI125_include(i))+Treshold_tAft);
                scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Cor.wait_start(CorITI125_include(i)),repmat(n,numel(spnum),1)',0.3,'y','filled');
                scatter(-eventVideoBehavCor.treshold(CorITI125_include(i)),repmat(n,numel(1),1)',0.3,'r','filled');
                %scatter(-eventVideoBehavCor.treshold(CorITI125_include(i))+(eventTimes.Cor.resp(CorITI125_include(i))-eventTimes.Cor.trial_start(CorITI125_include(i))),repmat(n,numel(1),1)',0.5,'b','filled');
                n = n+1;
            end
%             for i=1:numel(TrialDistr.PremTresholdIncluded)
...
%             end
            ...
            n = 1;
%             for i=1:numel(CorITI5_include)
%                 spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.cue(CorITI5_include(i))-Cue_tBef & All{21, 1}{neuron, 1} < eventTimes.Cor.cue(CorITI5_include(i))+Cue_tAft);
%                 scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Cor.cue(CorITI5_include(i)),repmat(n,numel(spnum),1)',0.3,'k','filled');
%                 n = n+1;
%             end
%             for i=1:numel(CorITI75_include)
%                 spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.cue(CorITI75_include(i))-Cue_tBef & All{21, 1}{neuron, 1} < eventTimes.Cor.cue(CorITI75_include(i))+Cue_tAft);
%                 scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Cor.cue(CorITI75_include(i)),repmat(n,numel(spnum),1)',0.3,'c','filled');
%                 n = n+1;
%             end
            for i=1:numel(CorITI125_include)
                spnum=find(All{21, 1}{neuron, 1}>eventTimes.Cor.cue(CorITI125_include(i))-Cue_tBef & All{21, 1}{neuron, 1} < eventTimes.Cor.cue(CorITI125_include(i))+Cue_tAft);
                scatter(All{21, 1}{neuron, 1}(spnum)-eventTimes.Cor.cue(CorITI125_include(i)),repmat(n,numel(spnum),1)',0.3,'y','filled');
                n = n+1;
            end
